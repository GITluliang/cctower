 ...
 */
@Controller
@RequestMapping("/mobile/car/special")
public class MobileCarSpecialController {
    private String prefix = "h5/mobile/special/";
    @Autowired
    private CarService carService;

    @RequestMapping()
    public String carLong() {
        return prefix + "listSpecial";
    }

    @ResponseBody
    @RequestMapping("authz")
    public Result carLongAuthZ() {
        return getSubject().isPermitted("sys:car:special") ? ResponseResult.success() : ResponseResult.fail(401, "没有权限");
    }
    @RequestMapping("add")
    public String carVIPAdd() {
        return prefix + "addSpecial";
    }

    @RequestMapping("detail/{id}")
    public String carVIPDetail(@PathVariable Long id, Model model) {
        model.addAttribute(id);
        return prefix + "detailSpecial";
    }

    @RequestMapping("/edit/{id}")
    public String carVIPEdit(@PathVariable Long id, Model model) {
        model.addAttribute(carService.findById(id));
        return prefix + "editSpecial";
    }
}
